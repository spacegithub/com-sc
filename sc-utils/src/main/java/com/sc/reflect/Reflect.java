...
/**
 * usge:Reflect.on(UserManager.class).call("get", getApplicationContext());
 ...
 * some example
 * 创建实例
 * 1.String string = Reflect.on(String.class).create("Hello World").get();
 * <p>
 * 访问属性（public,protected,package,private均可）
 * 1.char pathSeparatorChar = Reflect.on(File.class).create("/sdcard/droidyue.com").field("pathSeparatorChar").get();
 * 修改属性(final属性也可以修改)
 * <p>
 * 1.String setValue = Reflect.on(File.class).create("/sdcard/drodiyue.com").set("path", "fakepath").get("path");
 * <p>
 * 调用方法（public,protected,package,private均可）
 * 1.ArrayList arrayList = new ArrayList();
 * 2.arrayList.add("Hello");
 * 3.arrayList.add("World");
 * 4.int value = Reflect.on(arrayList).call("hugeCapacity", 12).get();
 */
